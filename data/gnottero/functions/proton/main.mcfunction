#> This is the main function, that will run once per tick
    ...
    #> Call the "gnottero:proton/events/main" function to handle all the datapack events
        execute as @e[type=minecraft:armor_stand, nbt={NoGravity:1b, Invisible:1b, Small:1b, Tags:["gn.proton_crafting"]}] at @s run function gnottero:proton/events/block/main
