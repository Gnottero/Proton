#> Check if the player is performing a recipe and handle the event
    ...
    #> Check if the player remove the recipe
        ...
        execute if score #gn.recipe_length gn.proton_check matches 0..9 if score #gn.found_recipe gn.proton_check matches 0 unless entity @s[tag=gn.is_crafting] if data storage gnottero:proton/block GuiHandler[{Slot:15b}] run function gnottero:proton/events/block/gui/check_slots/invalid_result 
