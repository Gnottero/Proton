#> Store the item in the "gnottero:proton/block" storage and set the placeholder in its place
    ...
    #> Store the data and draw the item
        execute if data storage gnottero:proton/block ProtonGui[{Slot: 6b}].Count run data modify storage gnottero:proton/block drop_items insert -1 from storage gnottero:proton/block ProtonGui[{Slot: 6b}]
