#> Summon the item in order to give it back to the player
    ...
    #> Store the item data inside the placeholder
        data modify entity @e[type=minecraft:item, distance=..1.2, limit=1, tag=gn.proton_placeholder, nbt={Item:{id:"minecraft:stone"}}] Item set from storage gnottero:proton/block GuiItems[0]

    #> Remove the last element from the "gnottero:proton/block" storage
        data remove storage gnottero:proton/block GuiItems[0]
