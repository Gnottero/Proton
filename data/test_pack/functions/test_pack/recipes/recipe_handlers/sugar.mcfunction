#> Handle the recipe functions. Call the right function either to add the result or to reduce the item count
    ...
    #> Set the result in the "RecipeHandler.RecipeOutputItem" storage
        data modify storage gnottero:proton/block RecipeHandler.RecipeOutputItem set value {Slot:15b, id:"minecraft:sugar", Count:1b}

    #> Set the result
        execute if data storage gnottero:proton/block RecipeHandler.RecipeOutputItem.Count unless data storage gnottero:proton/block RecipeHandler.RecipeOutputItem{Slot:15b, id:"minecraft:sugar", Count:1b} run function gnottero:proton/events/block/gui/check_slots/invalid_result
